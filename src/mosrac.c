#include "mosrac.h"
// #include "main.h"
#include <Arduino.h>

#ifdef __cplusplus
extern "C" {
#endif

static const uint8_t crcTable[256] = {
    
};

uint8_t ML_ENC_CalcCRC(volatile uint8_t *buffer, uint8_t length) {
    if (length == 0) return 0;

    uint8_t *p = (uint8_t *)buffer;
    uint8_t temp = *p++;
    
    while(--length) {
        temp = *p++ ^ crcTable[temp];
    }
    return crcTable[temp];
}

uint8_t Get_CRC_position(uint8_t command){
    uint8_t offset = 0;

    switch (command) {
        case MBS_CMD_SET_ZERO:        offset = 1; break;
        case MBS_CMD_GET_POS:         offset = 5; break;
        case MBS_CMD_GET_POS_STATUS:  offset = 6; break;
        case MBS_CMD_GET_POS_RPM:     
        case MBS_CMD_GET_POS_TEMP:    offset = 7; break;
        default: return 0;
    }
    return offset;
}


uint8_t Get_Packet_CRC(uint8_t command, uint8_t *raw) {
    // volatile uint8_t *data = (volatile uint8_t *)raw;
    return raw[Get_CRC_position(command)];
}

uint8_t CRC_is_OK(uint8_t command, uint8_t *data)
{
    // uint8_t *data = (uint8_t *)pStruct;
    uint8_t data_len = Get_CRC_position(command);
    uint8_t calculated_crc = ML_ENC_CalcCRC(data, data_len);
    uint8_t received_crc = data[data_len];
    // if(received_crc != calculated_crc) printf("CRC ERROR\r\n");
    return (received_crc == calculated_crc);
}

ENC_Diag_t Decode_Encoder_Status(uint8_t raw_byte) {
    ENC_Status_t s = *(ENC_Status_t*)&raw_byte;
    ENC_Diag_t diag = {0};
    
    diag.is_error = s.error;
    diag.is_warning = s.warning;

    if (s.error) {
        if (s.over_speed)           strcpy(diag.msg, "Error: Over Speed");
        else if (s.temp_out)        strcpy(diag.msg, "Error: Temperature");
        else if (s.mag_weak)        strcpy(diag.msg, "Error: Magnet Weak");
        else if (s.mag_large)       strcpy(diag.msg, "Error: Magnet Large");
        else if (s.battery_lost)    strcpy(diag.msg, "Error: Multiturn LOST (Power Failure)");
        else if (s.battery_low)     strcpy(diag.msg, "Error: Battery < 2.7V (Critical)");
    }
    else if (s.warning) {
        if (s.over_speed)           strcpy(diag.msg, "Warning: Over Speed");
        else if (s.mag_weak)        strcpy(diag.msg, "Warning: Magnet Weak");
        else if (s.mag_large)       strcpy(diag.msg, "Warning: Magnet Large");
        else if (s.battery_low)     strcpy(diag.msg, "Warning: Battery < 2.9V (Low)");
    }
    else {strcpy(diag.msg, "Status OK");}

    return diag;
}


/*!SECTION

float readMySensorCallback(void){
	uint8_t mbs_cmd = MBS_CMD_GET_POS_STATUS;
    RS485_tx(mbs_cmd, UART_ENC);
	UART_ENC.readBytes(rx_buffer_enc, 10);
	// TOFRcv.RAW = rx_buffer_enc;
	// strcpy(TOFRcv.RAW, rx_buffer_enc);

	ENC_Diag_t result = Decode_Encoder_Status(rx_buffer_enc[5]);

	printf("turns:%d \t count %d \t angle %f \t rad %f \t  shaft %f \t CRC: %02X/%02X status:%02X %s\r\n",
              Get_Multiturn(rx_buffer_enc),
              Get_Singleturn(rx_buffer_enc),
              Get_angle_degree(rx_buffer_enc),
              Get_angle_radian(rx_buffer_enc),
              Get_angle_shaft(Get_angle_degree(rx_buffer_enc),Get_Multiturn(rx_buffer_enc), 47),
              Get_Packet_CRC(mbs_cmd,rx_buffer_enc),
              ML_ENC_CalcCRC(rx_buffer_enc, Get_CRC_position(mbs_cmd)),
              Get_Status_Raw(rx_buffer_enc),
              result.msg
			      );

	// float angle_rad = 0.0f;
	// uint8_t mbs_cmd = MBS_CMD_GET_POS_STATUS;
	// RS485_tx(mbs_cmd, UART_ENC);
	// int bytesRead = UART_ENC.readBytes(rx_buffer_enc, 8);
	// if(CRC_is_OK(mbs_cmd, rx_buffer_enc))
	// {
	// 	angle_rad = Get_angle_radian(rx_buffer_enc);
	// }
	return Get_angle_radian(rx_buffer_enc);
}
	
*/

#ifdef __cplusplus
}
#endif