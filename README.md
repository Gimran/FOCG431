...
### Запуск GUI мышью, без терминала

```bash
bash tools/install_launcher.sh ~/Desktop/py_gui_test/can_gui.py
```

Скрипт ставит ярлык `SimpleFOC CAN Studio` с иконкой в трёх местах: в меню
приложений (поиск по слову «SimpleFOC»), на рабочий стол и в папку рядом с
самим `can_gui.py`. Терминал при запуске не открывается. Аргумент можно
опустить — тогда возьмётся `can_gui.py` из корня репозитория.

Ярлык на рабочем столе GNOME запускается только помеченным доверенным; скрипт
делает это через `gio`, но если значок всё равно не реагирует — правый клик и
**Allow Launching**.

Иконка лежит в `tools/icon.png`, пересобирается `python3 tools/make_icon.py`.

Это не самостоятельный бинарник: на машине должны быть Python и зависимости
GUI (`customtkinter`, `python-can`, `matplotlib`, `pysimplefoc`). Для запуска
на машине без них понадобился бы PyInstaller — образ вышел бы на сотни
мегабайт из-за matplotlib, поэтому по умолчанию сделан обычный ярлык.
